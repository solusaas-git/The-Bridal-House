...
export default function EditFittingPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        setLoading(true);
        const [fitRes, c, p, r] = await Promise.all([
          axios.get(`/api/fittings/${id}`),
        ...
        if (fit) {
          setSelectedClient(fit.client || null);
          setClientSearch(fit.client ? `${fit.client.firstName} ${fit.client.lastName}` : '');
          setSelectedItems(fit.items || []);
          setFittingDate(fit.pickupDate ? String(fit.pickupDate).substring(0, 10) : '');
          setFittingTime(fit.pickupDate ? String(fit.pickupDate).substring(11, 16) : '15:00');
          setNotes(fit.notes || '');
          setStatus((fit.status as any) || 'Confirmed');
        }
      } catch (e) {
  ...
  const getCategories = useCallback(() => {
    ...
    products.forEach((p: any) => {
      const name = typeof p.category === 'object' && (p.category as any)?.name ? (p.category as any).name : p.category;
      if (name) set.add(name as string);
    });
    return Array.from(set);
  }, [products]);

  // Availability logic: product unavailable only if booked on same wedding date
  ...
  const getFilteredProducts = useCallback(() => {
    return products.filter((product: any) => {
      ...
      const matchesCategory = selectedCategoryTab === 'all' || (() => {
        const cat = typeof product.category === 'object' && (product.category as any)?.name ? (product.category as any).name : product.category;
        return cat === selectedCategoryTab;
      })();
      const matchesSearch = !itemSearchTerm || product.name?.toLowerCase().includes(itemSearchTerm.toLowerCase()) || (() => {
        const cat = typeof product.category === 'object' && (product.category as any)?.name ? (product.category as any).name : product.category;
  ...
  const handleSave = async () => {
    if (!selectedClient || !fittingDate) return;
    try {
      await axios.put(`/api/fittings/${id}` , {
    ...
    <Layout>
      <div className="p-4 sm:p-6 max-w-5xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          ...
          <a href={`/fittings/${id}`} className="p-2 hover:bg-white/10 rounded-lg transition-colors text-gray-300">{t('common.cancel')}</a>
        </div>

        <div className="bg-white/10 backdrop-blur-lg border border-white/10 rounded-xl p-4 sm:p-6">
          {/* Client selection */}
          <div className="space-y-3 mb-6">
            <h3 className="text-lg font-medium text-white">{t('common.client')}</h3>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                <MagnifyingGlassIcon className="h-5 w-5 text-white/50" />
              </div>
              <input type="text" placeholder={t('edit.searchCustomersPlaceholder') as string} value={clientSearch} onChange={(e) => setClientSearch(e.target.value)} className="block w-full pl-10 pr-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" />
              {showClientDropdown && (
          ...
          <div className="bg-white/5 rounded-lg p-4 border border-white/10 mb-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-gray-300">{t('common.fittingTime')} *</label>
                <div className="relative">
                  <CalendarIcon className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400" />
                  <input type="time" value={fittingTime} onChange={(e) => setFittingTime(e.target.value)} className="w-full pl-8 pr-2.5 py-2 bg-white/5 border border-white/10 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
                </div>
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-300">{t('common.fittingDate')} *</label>
                <div className="relative">
                  <CalendarIcon className="absolute left-2.5 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-gray-400" />
                  <input type="date" value={fittingDate} onChange={(e) => setFittingDate(e.target.value)} className="w-full pl-8 pr-2.5 py-2 bg-white/5 border border-white/10 rounded-md text-white focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm" />
                </div>
              </div>
            </div>
            <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-medium text-gray-300">{t('common.status')}</label>
            ...
            <div className="mt-4">
              <label className="block text-xs font-medium text-gray-300">{t('common.notes')}</label>
              <textarea
                value={notes}
          ...
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <select value={availabilityFilter} onChange={(e) => setAvailabilityFilter(e.target.value as any)} className="px-2 sm:px-3 py-1 sm:py-2 bg-white/10 border border-white/20 rounded-md text-white text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            ...
            <div className="overflow-x-auto pb-2 -mx-4 px-4 sm:mx-0 sm:px-0 mb-3">
              <div className="flex gap-2 min-w-max sm:flex-wrap">
                <button onClick={() => setSelectedCategoryTab('all')} className={`px-3 py-2 sm:px-4 rounded-lg text-sm font-medium transition-all whitespace-nowrap flex-shrink-0 ${selectedCategoryTab === 'all' ? 'bg-blue-500 text-white shadow-lg' : 'bg-white/5 border border-white/10 text-gray-300 hover:bg-white/10'}`}>{t('filters.availability.all')} ({filteredProducts.length})</button>
                {getCategories().map((c) => (
                    ...
                    <div className="p-3">
                      {imgSrc ? (
                        <div className="relative">
                          ...
                          />
                          <button type="button" className="absolute top-2 left-2 p-1.5 bg-black/50 hover:bg-black/70 text-white rounded-md" onClick={(e) => { e.stopPropagation(); openImageModal(imgSrc, p.name); }} title={t('common.preview') as string}>
          ...
          {/* Actions */}
          <div className="flex justify-end mt-6 pt-4 border-t border-white/10 gap-3">
            <a href={`/fittings/${id}`} className="px-4 py-2 rounded-lg text-white text-sm font-medium transition-colors bg-white/10 hover:bg-white/20">{t('common.cancel')}</a>
            <button type="button" onClick={handleSave} disabled={!selectedClient || !fittingDate} className="px-4 py-2 rounded-lg text-white text-sm font-medium transition-colors bg-blue-500 hover:bg-blue-600 disabled:opacity-50">{t('edit.save')}</button>
          </div>
        </div>
      </div>
      ...
      {imagePopup.show && !imagePopup.isModal && typeof window !== 'undefined' && (
        <div className="fixed pointer-events-none z-[9999]" style={{ left: `${imagePopup.x}px`, top: `${imagePopup.y}px`, transform: 'translateY(-50%)' }}>
